"""
Cooking Experiment 10: onion rollback + new required cumin appears for
Qwen2.5-VL + Franka Panda + MuJoCo + manual strategy-template shielding.

Experiment
----------
The robot starts with the normal seven-ingredient partial-order cooking recipe:

    onion + garlic -> tomato -> turmeric/coriander/salt -> cream -> finish

Two controlled disturbances are applied:

1. Immediately after Qwen successfully executes add:onion, the environment moves
   onion back out of the mixing bowl and onto the table. The previous-command
   history is not edited, so history says onion was executed while the current
   MuJoCo scene says onion is missing again.

2. After tomato is successfully added, a new brown cumin-powder container appears
   on the table and the task requirement changes. Cumin becomes a newly required
   spice, so the updated recipe becomes:

    onion + garlic -> tomato -> turmeric/coriander/salt/cumin -> cream -> finish

Before the cumin disturbance, `add:cumin` is represented in the action space but
is unsafe because cumin is not yet part of the recipe. After the disturbance,
`add:cumin` becomes part of the live spice-stage obligation until cumin is added.

The prompt uses the minimal Experiment 10 update: once cumin appears, it updates
only the visual legend and the Stage 3 spice rule. It does not repeatedly tell
the model that cumin is required.
...
SCENE_PATH = Path(".cooking_exp10_rollback_cumin_scene.generated.xml").resolve()
CAMERA_NAME = "table_cam"

CUMIN_ITEM = "cumin"
CUMIN_ACTION_LABEL = "add:cumin"
CUMIN_HALF_HEIGHT = 0.045
CUMIN_HIDDEN_POS = np.array([2.0, 2.0, TABLE_TOP_Z + CUMIN_HALF_HEIGHT], dtype=float)
CUMIN_TABLE_POS = np.array([0.87, -0.20, TABLE_TOP_Z + CUMIN_HALF_HEIGHT], dtype=float)
CUMIN_BOWL_POS = np.array([0.69, 0.11, TABLE_TOP_Z + CUMIN_HALF_HEIGHT + 0.012], dtype=float)
CUMIN_SPICE_ITEMS = ("turmeric", "coriander", "salt", CUMIN_ITEM)
ROLLBACK_ITEM = "onion"
ROLLBACK_TABLE_POS = np.array(INGREDIENTS[ROLLBACK_ITEM]["start_pos"], dtype=float).copy()
IMAGE_HEIGHT = 224
IMAGE_WIDTH = 224

VALID_ACTIONS = {"add", "stir", "finish"}
VALID_ITEMS = set(INGREDIENT_ORDER) | {CUMIN_ITEM}



def ensure_exp10_scene() -> Path:
    """Generate an Experiment 10 scene from the existing cooking XML.

    The cumin body exists in the generated model but starts hidden far outside the
    workspace. Onion rollback reuses the existing onion body. After tomato is
    successfully added, the experiment moves cumin onto the visible tabletop and
    marks cumin as a required spice.
    """
    if not BASE_SCENE_PATH.exists():
        raise FileNotFoundError(
            f"Missing {BASE_SCENE_PATH}. Experiment 10 reuses the existing cooking scene."
        )

    base_xml = BASE_SCENE_PATH.read_text(encoding="utf-8")
    if 'body name="cumin"' not in base_xml:
        cumin_body = (
            "\n\n    <!-- Experiment 10 dynamic ingredient: hidden initially, appears after tomato. -->\n"
            f"    <body name=\"cumin\" pos=\"{CUMIN_HIDDEN_POS[0]} {CUMIN_HIDDEN_POS[1]} {CUMIN_HIDDEN_POS[2]}\">\n"
            "      <freejoint/>\n"
            f"      <geom name=\"cumin_geom\" type=\"cylinder\" size=\"0.032 {CUMIN_HALF_HEIGHT}\"\n"
            "            rgba=\"0.55 0.32 0.10 1\" mass=\"0.07\"/>\n"
            "    </body>"
        )
        close_tag = "  </worldbody>"
        if close_tag not in base_xml:
            raise RuntimeError("Could not find </worldbody> insertion point in base cooking scene.")
        base_xml = base_xml.replace(close_tag, cumin_body + "\n" + close_tag, 1)

    SCENE_PATH.write_text(base_xml, encoding="utf-8")
    return SCENE_PATH


def _normalize_exp10(distribution: Mapping[str, float]) -> Dict[str, float]:
    clipped = {action: max(0.0, float(value)) for action, value in distribution.items()}
    total = sum(clipped.values())
    if total <= 0.0:
        raise RuntimeError("Experiment 10 shield distribution has zero total probability mass.")
    return {action: value / total for action, value in clipped.items()}


def _body_location_and_added(
    ...
    if body_id < 0:
        return "missing_body", False
    pos = data.xpos[body_id].copy()
    if pos[0] > 1.5 and pos[1] > 1.5:
        return "hidden", False
    bowl_xy = np.asarray(BOWL_CENTER[:2], dtype=float)
    added = bool(np.linalg.norm(pos[:2] - bowl_xy) <= 0.16)
    return ("bowl" if added else "table"), added


def get_exp10_task_state(
    model: mujoco.MjModel,
    data: mujoco.MjData,
    cumin_required: bool,
) -> Dict[str, Dict[str, Any]]:
    state = get_task_state(model, data)
    location, added = _body_location_and_added(model, data, CUMIN_ITEM)
    state[CUMIN_ITEM] = {
        "location": location,
        "added": added,
        "required": bool(cumin_required),
        "visible": location != "hidden",
    }
    return state


def active_stage_exp10(task_state: Dict[str, Dict[str, Any]]) -> Tuple[str, Tuple[str, ...]]:
    aromatics = tuple(item for item in ("onion", "garlic") if not task_state[item]["added"])
    if aromatics:
        return "aromatics", aromatics
    if not task_state["tomato"]["added"]:
        return "tomato", ("tomato",)

    cumin_required = bool(task_state.get(CUMIN_ITEM, {}).get("required", False))
    spice_items = CUMIN_SPICE_ITEMS if cumin_required else CUMIN_SPICE_ITEMS[:-1]
    missing_spices = tuple(item for item in spice_items if not task_state[item]["added"])
    if missing_spices:
        return "spices", missing_spices

    if not task_state["cream"]["added"]:
        return "cream", ("cream",)
    return "complete", tuple()


def task_complete_exp10(task_state: Dict[str, Dict[str, Any]]) -> bool:
    _, incomplete = active_stage_exp10(task_state)
    return len(incomplete) == 0


def abstract_state_key_exp10(task_state: Dict[str, Dict[str, Any]]) -> Tuple[bool, ...]:
    return tuple(bool(task_state[item]["added"]) for item in INGREDIENT_ORDER) + (
        bool(task_state.get(CUMIN_ITEM, {}).get("required", False)),
        bool(task_state.get(CUMIN_ITEM, {}).get("added", False)),
    )


def hard_unsafe_actions_exp10(task_state: Dict[str, Dict[str, Any]]) -> Tuple[str, ...]:
    stage_name, live_items = active_stage_exp10(task_state)
    live_actions = {f"add:{item}" for item in live_items}
    if stage_name == "complete":
        live_actions = {"finish"}

    actions = tuple([*ACTION_LABELS, CUMIN_ACTION_LABEL])
    unsafe: List[str] = []
    for label in actions:
        if label == "stir":
            continue
        if label not in live_actions:
            unsafe.append(label)

    if not bool(task_state.get(CUMIN_ITEM, {}).get("required", False)) and CUMIN_ACTION_LABEL not in unsafe:
        unsafe.append(CUMIN_ACTION_LABEL)

    return tuple(sorted(set(unsafe)))


def classify_action_exp10(task_state: Dict[str, Dict[str, Any]], label: str) -> str:
    stage_name, live_items = active_stage_exp10(task_state)
    live_actions = {f"add:{item}" for item in live_items}
    if stage_name == "complete":
        live_actions = {"finish"}
    if label in hard_unsafe_actions_exp10(task_state):
        return "unsafe"
    if label == "stir":
        return "co_live"
    if label in live_actions:
        return "live_progress"
    return "other_allowed"


def print_task_state_exp10(task_state: Dict[str, Dict[str, Any]]) -> None:
    print("\n===== Cooking Task State =====")
    for item in INGREDIENT_ORDER:
        info = task_state[item]
        mark = "✅" if info["added"] else "❌"
        print(f"{mark} {item:<10} | location={info['location']:<5} | added={info['added']}")

    cumin = task_state[CUMIN_ITEM]
    if cumin["required"] or cumin["visible"]:
        mark = "✅" if cumin["added"] else "❌"
        required_text = "required" if cumin["required"] else "not-required"
        print(
            f"{mark} {CUMIN_ITEM:<10} | location={cumin['location']:<5} | "
            f"added={cumin['added']} | {required_text}"
        )

    stage_name, incomplete = active_stage_exp10(task_state)
    print("--------------------------------")
    print("Active stage:", stage_name)
    print("Incomplete in active stage:", list(incomplete))
    print("Task complete:", task_complete_exp10(task_state))
    print("================================\n")


class Exp10CuminStrategyTemplateGuide(ManualCookingStrategyTemplateGuide):
    """Automaton-backed guide for rollback plus dynamic cumin."""

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        kwargs["action_labels"] = tuple([*ACTION_LABELS, CUMIN_ACTION_LABEL])
...
class FrankaCookingSimulator:
    ...
    def update_held_object(self, held_item: Optional[str]) -> None:
        ...
        half_height = (
            CUMIN_HALF_HEIGHT
            if held_item == CUMIN_ITEM
            else float(INGREDIENTS[held_item]["half_height"])
        )
    ...
    def place_in_bowl(self, item: str) -> None:
        ...
        place_pos = CUMIN_BOWL_POS.copy() if item == CUMIN_ITEM else bowl_place_position(item)
...
def make_planner_prompt(
    ...
    # Minimal Experiment 10 prompt update:
    # - Before cumin appears: no cumin visual legend, and the original spice rule is shown.
    # - After cumin appears: add only the cumin visual legend and the updated Stage 3 spice rule.
    # This avoids repeatedly emphasizing "cumin is required" after cumin has already been added.
    if cumin_required:
        cumin_legend_line = "- Brown cylinder: cumin powder.\n"
        recipe_stage_3 = (
            "Stage 3: after tomato is added, add turmeric, coriander powder, and "
            "cumin powder. These required core spices may be added in ANY order."
        )
    else:
        cumin_legend_line = ""
        recipe_stage_3 = (
            "Stage 3: after tomato is added, add turmeric and coriander powder. "
            "These required core spices may be added in either order."
        )
...
{cumin_legend_line}- Large blue circular region in the center: mixing bowl.
...
def maybe_apply_disturbances(
    args: argparse.Namespace,
    model: mujoco.MjModel,
    data: mujoco.MjData,
    sim: FrankaCookingSimulator,
    last_executed_action: Dict[str, Any],
    onion_rollback_applied: bool,
    cumin_disturbance_applied: bool,
) -> Tuple[bool, bool]:
    """Apply the two Experiment 10 disturbances when their triggers occur.

    Disturbance 1: after add:onion, move onion back to the table.
    Disturbance 2: after add:tomato, reveal cumin and make it required.

    The previous-command history is intentionally not edited for either
    disturbance. This preserves the history/current-scene conflict for rollback
    and lets the prompt update only through the current scene/task rule.
    """
    del args

    if (
        not onion_rollback_applied
        and last_executed_action.get("action") == "add"
        and last_executed_action.get("item") == ROLLBACK_ITEM
    ):
        current_state = get_task_state(model, data)
        print("\n*** COOKING EXPERIMENT 10 DISTURBANCE 1 APPLIED ***")
        print("Mode: onion_rollback_to_table")
        print("Trigger: immediately after robot successfully executed add:onion")
        print("Environment action: move onion back out of the mixing bowl onto the table")
        print("Previous-command history changed: False")
        print("Shield state source of truth: current MuJoCo scene")

        if not bool(current_state[ROLLBACK_ITEM]["added"]):
            print("Onion was not detected in the bowl at rollback time; no physical move needed.")
            sim.return_home()
        else:
            target_pos = ROLLBACK_TABLE_POS.copy()
            print("Moving onion back to table position:", target_pos.tolist())
            set_body_pose(model, data, ROLLBACK_ITEM, target_pos)
            sim.return_home()
        onion_rollback_applied = True

    if (
        not cumin_disturbance_applied
        and last_executed_action.get("action") == "add"
        and last_executed_action.get("item") == "tomato"
    ):
        print("\n*** COOKING EXPERIMENT 10 DISTURBANCE 2 APPLIED ***")
        print("Mode: new_required_cumin_appears")
        print("Trigger: after tomato was successfully added")
        print("New visible object: cumin powder (brown cylinder)")
        print("Recipe requirement changed: True")
        print("Updated spice-stage requirement: turmeric, coriander, salt, and cumin")
        print("Shield classification of add:cumin: live when cumin is missing in spice stage")
        print("Previous-command history changed: False")
        print("Moving cumin onto table position:", CUMIN_TABLE_POS.tolist())
        set_body_pose(model, data, CUMIN_ITEM, CUMIN_TABLE_POS)
        sim.return_home()
        cumin_disturbance_applied = True

    return onion_rollback_applied, cumin_disturbance_applied
...
def run_episode(args: argparse.Namespace) -> bool:
    ...
    set_body_pose(model, data, CUMIN_ITEM, CUMIN_HIDDEN_POS)
    ...
    action_labels = tuple([*ACTION_LABELS, CUMIN_ACTION_LABEL])
    automaton = CookingAutomaton(
    ...
    if args.shield_mode == "template_guidance":
        guide = Exp10CuminStrategyTemplateGuide(
    ...
    onion_rollback_applied = False
    cumin_disturbance_applied = False
    ...
    try:
        ...
        print("\n" + "=" * 72)
        print("COOKING EXPERIMENT 10: ONION ROLLBACK + NEW REQUIRED CUMIN")
        print("After onion is executed once, onion is moved back to the table.")
        print("After tomato is successfully added, cumin appears and becomes required.")
        print("=" * 72)
        print("\nInitial cooking task state:")
        print_task_state_exp10(get_exp10_task_state(model, data, cumin_disturbance_applied))
        sim.return_home()
        ...
        for step_index in range(episode_step_limit):
            ...
            mujoco.mj_forward(model, data)
            task_state = get_exp10_task_state(model, data, cumin_disturbance_applied)
            ...
            onion_rollback_applied, cumin_disturbance_applied = maybe_apply_disturbances(
                args=args,
                model=model,
                data=data,
                sim=sim,
                last_executed_action=action,
                onion_rollback_applied=onion_rollback_applied,
                cumin_disturbance_applied=cumin_disturbance_applied,
            )
        ...
        final_state = get_exp10_task_state(model, data, cumin_disturbance_applied)
        ...
        print("=" * 72)
        print_task_state_exp10(final_state)
        ...
        print("Onion rollback applied:", onion_rollback_applied)
        print("Required cumin appeared:", cumin_disturbance_applied)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Cooking Experiment 10: onion rollback plus new required cumin with manual STARs-style strategy-template guidance."
    )
    parser.add_argument(
        "--planner",
    ...
    parser.add_argument(
        "--run-id",
        default="qwen25_cooking_exp10_onion_rollback_plus_cumin",
    )
    parser.add_argument(
        "--trace-jsonl",
        default="logs_model_reasoning/qwen25_cooking_exp10_onion_rollback_plus_cumin.jsonl",
    )
    parser.add_argument(
        "--shield-trace-jsonl",
        default="logs_model_reasoning/qwen25_cooking_exp10_onion_rollback_plus_cumin_shield.jsonl",
    )
    parser.add_argument(
        "--image-dir",
        default="planner_images_qwen25_cooking_exp10_onion_rollback_plus_cumin",
    )

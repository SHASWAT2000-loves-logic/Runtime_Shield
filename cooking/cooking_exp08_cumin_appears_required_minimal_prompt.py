"""
Cooking Experiment 8: new required cumin appears for
...
After tomato is successfully added, a new brown cumin-powder container appears on
the table and the task requirement changes: cumin becomes a newly required spice.
The updated recipe becomes:

    onion + garlic -> tomato -> turmeric/coriander/salt/cumin -> cream -> finish

Before the disturbance, `add:cumin` is represented in the action space but is
unsafe because cumin is not yet part of the recipe. After the disturbance,
`add:cumin` becomes part of the live spice-stage obligation until cumin is added.
The shield rejects old-recipe behavior such as adding cream or finishing before
cumin has been added.
...
SCENE_PATH = Path(".cooking_exp08_cumin_scene.generated.xml").resolve()
...
def ensure_exp08_scene() -> Path:
    """Generate an Experiment 8 scene from the existing cooking XML.

    The cumin body exists in the generated model but starts hidden far outside the
    workspace. After tomato is successfully added, the experiment moves it onto
    the visible tabletop and marks cumin as a required spice.
    """
    if not BASE_SCENE_PATH.exists():
        raise FileNotFoundError(
            f"Missing {BASE_SCENE_PATH}. Experiment 8 reuses the existing cooking scene."
        )

    base_xml = BASE_SCENE_PATH.read_text(encoding="utf-8")
    if 'body name="cumin"' not in base_xml:
        cumin_body = (
            "\n\n    <!-- Experiment 8 dynamic ingredient: hidden initially, appears after tomato. -->\n"
...
def _normalize_exp08(distribution: Mapping[str, float]) -> Dict[str, float]:
    clipped = {action: max(0.0, float(value)) for action, value in distribution.items()}
    total = sum(clipped.values())
    if total <= 0.0:
        raise RuntimeError("Experiment 8 shield distribution has zero total probability mass.")
...
def active_stage_exp08(task_state: Dict[str, Dict[str, Any]]) -> Tuple[str, Tuple[str, ...]]:
...
def task_complete_exp08(task_state: Dict[str, Dict[str, Any]]) -> bool:
    _, incomplete = active_stage_exp08(task_state)
    return len(incomplete) == 0


def abstract_state_key_exp08(task_state: Dict[str, Dict[str, Any]]) -> Tuple[bool, ...]:
...
def hard_unsafe_actions_exp08(task_state: Dict[str, Dict[str, Any]]) -> Tuple[str, ...]:
    stage_name, live_items = active_stage_exp08(task_state)
...
def classify_action_exp08(task_state: Dict[str, Dict[str, Any]], label: str) -> str:
    stage_name, live_items = active_stage_exp08(task_state)
    live_actions = {f"add:{item}" for item in live_items}
    if stage_name == "complete":
        live_actions = {"finish"}
    if label in hard_unsafe_actions_exp08(task_state):
        return "unsafe"
    if label == "stir":
        return "co_live"
...
def print_task_state_exp08(task_state: Dict[str, Dict[str, Any]]) -> None:
    print("\n===== Cooking Task State =====")
    ...
    stage_name, incomplete = active_stage_exp08(task_state)
    print("--------------------------------")
    print("Active stage:", stage_name)
    print("Incomplete in active stage:", list(incomplete))
    print("Task complete:", task_complete_exp08(task_state))
    print("================================\n")


class Exp08CuminStrategyTemplateGuide(ManualCookingStrategyTemplateGuide):
    """Automaton-backed guide whose cumin transition follows the observed requirement."""
...
def make_planner_prompt(
    ...
    # Minimal Experiment 8 prompt update:
...
def maybe_apply_disturbance(
    ...
    """After tomato is successfully added, make cumin appear and become required."""
    del args
    if already_applied:
        return True
    if last_executed_action.get("action") != "add" or last_executed_action.get("item") != "tomato":
        return False

    print("\n*** COOKING EXPERIMENT 8 DISTURBANCE APPLIED ***")
...
def run_episode(args: argparse.Namespace) -> bool:
    ...
    if args.shield_mode == "template_guidance":
        guide = Exp08CuminStrategyTemplateGuide(
    ...
    try:
        ...
        print("\n" + "=" * 72)
        print("COOKING EXPERIMENT 8: NEW REQUIRED CUMIN APPEARS")
        print("Cumin starts hidden. After tomato is successfully added, cumin appears")
        print("and becomes a required spice before cream/finish are allowed.")
        print("=" * 72)
        print("\nInitial cooking task state:")
        print_task_state_exp08(get_exp08_task_state(model, data, disturbance_applied))
        sim.return_home()
        ...
        for step_index in range(episode_step_limit):
            ...
            mujoco.mj_forward(model, data)
            task_state = get_exp08_task_state(model, data, disturbance_applied)
        ...
        final_state = get_exp08_task_state(model, data, disturbance_applied)
        ...
        print("=" * 72)
        print_task_state_exp08(final_state)
        ...
        print("Required cumin appeared:", disturbance_applied)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Cooking Experiment 8: new required cumin appears with manual STARs-style strategy-template guidance."
    )
    parser.add_argument(
        "--planner",
    ...
    parser.add_argument(
        "--run-id",
        default="qwen25_cooking_exp08_cumin_appears_required_minimal_prompt",
    )
    parser.add_argument(
        "--trace-jsonl",
        default="logs_model_reasoning/qwen25_cooking_exp08_cumin_appears_required_minimal_prompt.jsonl",
    )
    parser.add_argument(
        "--shield-trace-jsonl",
        default="logs_model_reasoning/qwen25_cooking_exp08_cumin_appears_required_minimal_prompt_shield.jsonl",
    )
    parser.add_argument(
        "--image-dir",
        default="planner_images_qwen25_cooking_exp08_cumin_appears_required_minimal_prompt",
    )

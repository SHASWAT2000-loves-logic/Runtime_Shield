"""
Partial-order cooking benchmark for Qwen2.5-VL + Franka Panda + MuJoCo.

Research goal
-------------
Demonstrate manual strategy-template shielding with minimal interference:
  * S (unsafe): hard recipe/order violations get blocked.
  * D (co-live): repeated `stir` actions are allowed but gradually penalized.
  * H_l (live): currently unlocked incomplete ingredients are progress actions.
  * If the active live-group probability mass crosses a guidance threshold,
    those candidate actions are explicitly inserted into a Qwen reprompt.
...
Example:
    python -u cooking_qwen25_stars.py \
      --planner qwen_ollama \
...
Model-serving or malformed-output failures stop the run immediately.
"""
...
SCENE_PATH = Path("cooking_stars_scene.xml").resolve()
...
def maybe_apply_disturbance(
    ...
    if already_applied or args.disturbance == "none":
        return already_applied
    if executed_action_count != args.disturb_after_step:
        return already_applied

    item = args.disturb_item
    if item not in INGREDIENTS:
        raise ValueError(f"Unknown disturbance ingredient: {item}")

    if args.disturbance == "rollback_ingredient":
        reset_pos = np.array(INGREDIENTS[item]["start_pos"], dtype=float)
        print("\n*** COOKING DISTURBANCE APPLIED ***")
        print("Mode: rollback_ingredient")
        print("Ingredient:", item)
        print("Moving ingredient back to table position:", reset_pos.tolist())
        set_body_pose(model, data, item, reset_pos)
        sim.return_home()
        print("\nTask state immediately after disturbance:")
...
def run_episode(args: argparse.Namespace) -> bool:
    if not SCENE_PATH.exists():
        raise FileNotFoundError(
            f"Missing {SCENE_PATH}. Copy cooking_stars_scene.xml into the current directory."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Partial-order cooking benchmark with manual STARs-style strategy-template guidance."
    )
    parser.add_argument(
        "--planner",
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Number of rejected Qwen proposals allowed before marking the experiment failed.",
    )
    ...
    parser.add_argument(
        "--disturbance",
        choices=["none", "rollback_ingredient"],
        default="none",
    )
    parser.add_argument(
        "--disturb-after-step",
        type=int,
        default=5,
        help="Apply disturbance after this many executed model actions.",
    )
    parser.add_argument(
        "--disturb-item",
        choices=sorted(VALID_ITEMS),
        default="turmeric",
    )

    parser.add_argument(
        "--run-id",
        default="qwen25_cooking_stars_template_guidance",
    )
    parser.add_argument(
        "--trace-jsonl",
        default="logs_model_reasoning/qwen25_cooking_stars_template_guidance.jsonl",
    )
    parser.add_argument(
        "--shield-trace-jsonl",
        default="logs_model_reasoning/qwen25_cooking_stars_shield_trace.jsonl",
    )
    parser.add_argument(
        "--image-dir",
        default="planner_images_qwen25_cooking_stars",
    )

"""
Cooking Experiment 9: onion rollback after prior execution for
Qwen2.5-VL + Franka Panda + MuJoCo + manual strategy-template shielding.

Experiment
----------
The normal partial-order recipe begins with seven required ingredients:

    onion + garlic -> tomato -> turmeric/coriander/salt -> cream -> finish

After the robot successfully executes `add:onion`, the environment rolls onion
back out of the mixing bowl and places it on the table again. The previous-command
history is NOT edited, so the prompt history still says onion was previously added,
while the current image/state shows onion back on the table. The shield uses the
current MuJoCo scene as the source of truth, so onion becomes live/missing again.

This tests whether Qwen can recover from a rollback where history and current scene
conflict. A compliant behavior is to add onion again before moving on to tomato.
If Qwen instead follows stale history and proposes tomato too early, the shield
rejects it, updates its STARs-style counters/probabilities, and eventually guides
Qwen toward the currently missing live action.
...
def ensure_exp09_scene() -> Path:
    """Experiment 9 reuses the existing cooking scene without adding new objects."""
    if not BASE_SCENE_PATH.exists():
        raise FileNotFoundError(
            f"Missing {BASE_SCENE_PATH}. Experiment 9 reuses the existing cooking scene."
        )
...
def maybe_apply_disturbance(
    ...
    """After onion is successfully added once, roll it back to the table.

    The previous-command history is intentionally not edited. This creates the
    intended history/current-scene conflict: history says onion was added, but the
    current MuJoCo scene says onion is missing again.
    """
    del args
    if already_applied:
        return already_applied

    if last_executed_action.get("action") != "add" or last_executed_action.get("item") != ROLLBACK_ITEM:
        return already_applied

    current_state = get_task_state(model, data)
    print("\n*** COOKING EXPERIMENT 9 DISTURBANCE APPLIED ***")
...
def run_episode(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        print("\n" + "=" * 72)
        print("COOKING EXPERIMENT 9: ONION ROLLBACK")
        print("After onion is executed once, the environment moves onion back to the table.")
        print("The previous-command history is not updated for this rollback action.")
        print("=" * 72)
        ...
        print("Onion rollback disturbance applied:", disturbance_applied)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Cooking Experiment 9: onion rollback with manual STARs-style strategy-template guidance."
    )
    parser.add_argument(
        "--planner",
    ...
    parser.add_argument(
        "--run-id",
        default="qwen25_cooking_exp09_onion_rollback",
    )
    parser.add_argument(
        "--trace-jsonl",
        default="logs_model_reasoning/qwen25_cooking_exp09_onion_rollback.jsonl",
    )
    parser.add_argument(
        "--shield-trace-jsonl",
        default="logs_model_reasoning/qwen25_cooking_exp09_onion_rollback_shield.jsonl",
    )
    parser.add_argument(
        "--image-dir",
        default="planner_images_qwen25_cooking_exp09_onion_rollback",
    )
